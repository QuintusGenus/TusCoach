...
export const generatePlan = async (days: number = 14): Promise<PlanOverview> => {
    const res = await client.post('/students/me/plan/generate', { days });
    return res.data;
};
...
export const updatePlanTask = async (taskId: number, targetMinutes: number) => {
    const res = await client.put(`/plan_tasks/${taskId}`, { target_minutes: targetMinutes });
    return res.data;
};
