    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.statsRow}>
          ...
          <View style={styles.statCard}>
            <Text style={[styles.statValue, { color: avgScore >= 50 ? '#16a34a' : '#ef4444' }]}>
              {avgScore.toFixed(1)}
            </Text>
      ...
      >
        ...
        {isLoading ? (
          ...
          <View style={styles.emptyState}>
            <FontAwesome name="line-chart" size={48} color="#d1d5db" />
                    ...
                    <View style={styles.breakdownSection}>
                      ...
                      {exam.breakdowns.map((b) => {
                        const color = b.subject
                          ? SUBJECT_COLORS[b.subject as TUSSubject] || '#6b7280'
                          : '#6b7280';
                        return (
                          <View key={b.id} style={styles.breakdownRow}>
                            ...
                            <Text style={[styles.bCol, styles.bNum, { color: '#ef4444' }]}>
                              {b.wrong}
                            </Text>
                            <Text style={[styles.bCol, styles.bNum, { color: '#9ca3af' }]}>
                              {b.blank}
                            </Text>
                            <Text
                              style={[
                                styles.bCol,
                                styles.bNum,
                                { fontWeight: '700', color: b.net >= 0 ? '#16a34a' : '#ef4444' },
                              ]}
                            >
                              {b.net.toFixed(1)}
                            </Text>
          ...
          <View style={styles.modalSheet}>
            ...
            <ScrollView style={styles.modalBody}>
              ...
              <View style={styles.netPreview}>
                ...
                <Text
                  style={[styles.netValue, { color: formNet >= 0 ? '#16a34a' : '#ef4444' }]}
                >
                  {formNet.toFixed(2)}
                </Text>
              </View>
  ...
  return (
    <View style={{ alignItems: 'center', marginTop: 8 }}>
      <Svg width={W} height={H}>
        {/* Y axis labels */}
        <SvgText x={PAD.left - 4} y={PAD.top + 4} fontSize={10} fill="#9ca3af" textAnchor="end">
          {maxS.toFixed(0)}
        </SvgText>
        <SvgText x={PAD.left - 4} y={PAD.top + chartH + 4} fontSize={10} fill="#9ca3af" textAnchor="end">
          {minS.toFixed(0)}
        </SvgText>
        ...
        {/* Dots + labels */}
        {points.map((p, i) => (
          <Circle key={`d${i}`} cx={p.x} cy={p.y} r={4} fill="#004225" />
        ))}
...
const styles = StyleSheet.create({
  ...
  statValue: { fontSize: 22, fontWeight: 'bold', color: '#004225' },
  ...
  examName: { fontSize: 16, fontWeight: '600', color: '#111' },
  examDate: { fontSize: 12, color: '#6b7280', marginTop: 2 },
  examScoreBadge: {
  ...
  examScoreText: { fontSize: 18, fontWeight: 'bold', color: '#004225' },
  examScoreLabel: { fontSize: 10, color: '#6b7280' },
  ...
  bCol: { flex: 1, fontSize: 12, textAlign: 'center', color: '#6b7280' },
  bDot: { width: 8, height: 8, borderRadius: 4 },
  bText: { fontSize: 12, color: '#374151', flexShrink: 1 },
  bNum: { fontSize: 13 },
  examNotes: {
    fontSize: 12,
    color: '#6b7280',
  ...
  bThText: { flex: 1, fontSize: 11, fontWeight: '600', color: '#9ca3af', textAlign: 'center' },
  bInputRow: {
    flexDirection: 'row',
    alignItems: 'center',
    paddingVertical: 4,
  ...
  bInputLabel: { fontSize: 12, color: '#374151' },
  bInput: {
    flex: 1,
    backgroundColor: '#f9fafb',
    borderWidth: 1,
